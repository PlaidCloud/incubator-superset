...
function SortIcon<D extends object>({
  ...
  let sortIcon = <FaSort />;
  if (isSorted) {
    sortIcon = isSorted === 'desc' ? <FaSortDesc /> : <FaSortAsc />;
  }
  return sortIcon;
}
...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  const renderGroupingHeaders = (): JSX.Element => {
    ...
    return (
      <tr
        css={css`
          th {
            border-right: 2px solid ${theme.colorBorderBg};
          }
          th:first-child {
            border-left: none;
          }
  ...
  const getColumnConfigs = useCallback(
    (column: DataColumnMeta, i: number): ColumnDef<D> & { name: string } => {
      const {
        key,
        label,
        isNumeric,
      ...
      return {
        ...
        cell: ({ getValue, row }) => {
          const value = getValue<DataRecordValue>();
