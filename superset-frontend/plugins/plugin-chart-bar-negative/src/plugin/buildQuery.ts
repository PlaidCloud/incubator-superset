...
import { BarNegativeFormData } from '../types';

export default function buildQuery(formData: BarNegativeFormData) {
  const { dimension, metrics } = formData;
  return buildQueryContext(formData, baseQueryObject => {
    ...
    const columns = ([dimension] as unknown[])
      .flat()
      .filter(c => (typeof c === 'string' ? c !== '' : Boolean(c))) as string[];
    const cleanMetrics = ((metrics ?? []) as unknown[]).filter(
      Boolean,
    ) as QueryFormMetric[];
    return [
      {
