...
Databend accepts a *leading* ``SETTINGS (...)`` clause before the statement --
e.g. ``SETTINGS (max_execute_time_in_seconds=300) SELECT ...`` -- as a
query-timeout wrapper. No built-in sqlglot dialect parses that form, so it is
absorbed here and re-emitted verbatim on generation.

The base is Postgres, matching ``databend-sqlalchemy``, whose ``DatabendCompiler``
and ``DatabendIdentifierPreparer`` derive from ``PGCompiler`` and
``PGIdentifierPreparer``: the SQL Superset compiles and the SQL this dialect
